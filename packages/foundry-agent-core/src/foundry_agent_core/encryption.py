...
def load_encryption_key(env_var: str = "SESSION_ENCRYPTION_KEY") -> bytes:
    """Load and validate an encryption key from environment.
    ...
    if len(raw) != 64:
        raise AgentCreationError(
            ...
            context={"env_var": env_var, "length": len(raw)},
        )
    try:
        return bytes.fromhex(raw)
    except ValueError as e:
        raise AgentCreationError(
            f"{env_var} is not valid hexadecimal",
            context={"env_var": env_var},
        ) from e
