...
from foundry_agent_core import _MAX_CONTENT_LEN, _MAX_QUERY_LEN, AgentRequest, AgentResponse
...
from foundry_agent_fastapi.models.responses import QueryAPIResponse
...
def test_query_api_request_between_old_and_new_query_bound_succeeds():
    """A query between the old 8,192 bound and the new canonical bound now succeeds."""
    query = "x" * 100_000
    req = QueryAPIRequest(query=query)
...
def test_query_api_response_content_oversized_rejected():
    """Test that content exceeding the canonical bound is rejected."""
    with pytest.raises(PydanticValidationError):
        QueryAPIResponse(
            content="x" * (_MAX_CONTENT_LEN + 1),
            processing_time_ms=1.0,
            timestamp=datetime.now(),
        )


def test_query_api_response_content_at_max_tokens_ceiling_succeeds():
    """Content sized to the max_tokens=131072 ceiling (4:1 ratio) is accepted."""
    resp = QueryAPIResponse(
        content="x" * (131_072 * 4),
        processing_time_ms=1.0,
        timestamp=datetime.now(),
    )
    assert len(resp.content) == 131_072 * 4
...
def test_query_api_request_over_limit_maps_to_422():
    ...
    response = client.post("/query", json={"query": "x" * (_MAX_QUERY_LEN + 1)})
