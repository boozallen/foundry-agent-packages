...
"""Tests for wiring load_config into StrandsAgentConfig."""
...
class TestToolsDirLoading:
    """tools_dir is reachable through YAML, the load_config env override, and from_env."""

    def test_tools_dir_from_yaml(self, tmp_path: Path):
        yaml_path = tmp_path / "tools_dir.yaml"
        yaml_path.write_text(yaml.dump({"model": {"provider": "bedrock"}, "tools_dir": "/srv/agent/tools"}))

        config = load_config(StrandsAgentConfig, yaml_path, "STRANDS")

        assert config.tools_dir == "/srv/agent/tools"

    def test_tools_dir_env_override(self, minimal_yaml: Path, monkeypatch):
        monkeypatch.setenv("STRANDS__TOOLS_DIR", "/srv/override/tools")

        config = load_config(StrandsAgentConfig, minimal_yaml, "STRANDS")

        assert config.tools_dir == "/srv/override/tools"

    def test_tools_dir_env_override_matches_from_env(self, minimal_yaml: Path, monkeypatch):
        monkeypatch.setenv("STRANDS__TOOLS_DIR", "/srv/agent/tools")
        monkeypatch.setenv("STRANDS_TOOLS_DIR", "/srv/agent/tools")

        loaded = load_config(StrandsAgentConfig, minimal_yaml, "STRANDS")
        from_env = StrandsAgentConfig.from_env()

        assert loaded.tools_dir == from_env.tools_dir == "/srv/agent/tools"

    def test_tools_dir_absent_everywhere(self, minimal_yaml: Path, monkeypatch):
        monkeypatch.delenv("STRANDS__TOOLS_DIR", raising=False)
