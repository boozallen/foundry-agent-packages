...
class TestBedrockModelTemperature:
    """Tests for _default_bedrock_model temperature passthrough and the reasoning-model guard."""

    def test_temperature_passed_to_bedrock_model(self):
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {"model": {"model_id": "us.anthropic.claude-sonnet-4-20250514-v1:0", "temperature": 0.7}}
            _default_bedrock_model(config)
            call_kwargs = mock_cls.call_args[1]
            assert call_kwargs["temperature"] == 0.7

    def test_temperature_passed_for_open_model(self):
        # GPT-OSS-120B / NVIDIA Nemotron 3 Super 120B style Bedrock model_ids - accept temperature
        # normally, unaffected by the reasoning-model guard.
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {"model": {"model_id": "openai.gpt-oss-120b-1:0", "temperature": 0.3}}
            _default_bedrock_model(config)
            call_kwargs = mock_cls.call_args[1]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "us.anthropic.claude-mythos-5-20260101-v1:0",
            "us.anthropic.claude-mythos-preview-20260101-v1:0",
    ...
    def test_temperature_omitted_for_reasoning_locked_model(self, model_id):
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {"model": {"model_id": model_id, "temperature": 0.3}}
            _default_bedrock_model(config)
            call_kwargs = mock_cls.call_args[1]
            assert call_kwargs["temperature"] is None

    def test_temperature_passed_for_model_id_with_digit_suffix_collision(self):
        # A hypothetical distinct future model "claude-opus-50" must not be mistaken for
        # "claude-opus-5" via plain substring matching - the digit-boundary check in
        # _bedrock_model_rejects_temperature exists specifically to avoid this collision.
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {"model": {"model_id": "us.anthropic.claude-opus-50-20260101-v1:0", "temperature": 0.5}}
            _default_bedrock_model(config)
            call_kwargs = mock_cls.call_args[1]
            assert call_kwargs["temperature"] == 0.5

    def test_temperature_omitted_for_operator_configured_model(self):
        # bedrock_temperature_rejecting_models (StrandsAgentConfig.model / STRANDS_BEDROCK_
        # TEMPERATURE_REJECTING_MODELS) extends the built-in set - e.g. xai.grok-4.6, untested
        # and not in the built-in list, but an operator who confirmed it rejects temperature can
        # add it without a package upgrade.
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {
                "model": {
                    ...
                    "temperature": 0.5,
                    "bedrock_temperature_rejecting_models": ["xai.grok-4.6"],
                }
            }
            _default_bedrock_model(config)
            call_kwargs = mock_cls.call_args[1]
    ...
    def test_temperature_passed_for_unlisted_model_with_unrelated_configured_extras(self):
        # Configured extras that don't match this model_id don't affect it, and the built-in set
        # still applies independently of what's configured.
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            config = {
                "model": {
    ...
    def test_temperature_omitted_for_reasoning_locked_model_with_guardrails(self):
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            _default_bedrock_model(
                {
                    "model": {"model_id": "us.anthropic.claude-opus-5-20260101-v1:0", "temperature": 0.3},
                    "guardrail_config": {
                        "guardrail_id": "gr-123",
                        "guardrail_version": "1",
                        "guardrail_trace": "enabled",
    ...
    def test_temperature_none_when_not_configured(self):
        with patch("foundry_strands_agent.factory.BedrockModel") as mock_cls:
            mock_cls.return_value = MagicMock()
            _default_bedrock_model({"model": {"model_id": "us.anthropic.claude-sonnet-4-20250514-v1:0"}})
            call_kwargs = mock_cls.call_args[1]
...
class TestConfiguredToolsDirReachesLoader:
    """Spec: strands-tool-loading — Tools root is configurable."""

    @staticmethod
    def _config(tools_dir):
        from foundry_strands_agent.config.models import AgentModelConfig, StrandsAgentConfig

        return StrandsAgentConfig(
            model=AgentModelConfig(),
            tools_modules=["tools.calc"],
            tools_files=["/srv/tools/weather.py"],
            tools_dir=tools_dir,
        )

    @pytest.mark.asyncio
    async def test_configured_tools_dir_is_passed_to_both_loaders(self):
        factory = StrandsAgentFactory(container=MagicMock())
        registry = MagicMock()

        with (
            patch("foundry_strands_agent.factory.load_tool_from_module", new_callable=AsyncMock) as mock_module,
            patch("foundry_strands_agent.factory.load_tool_from_file", new_callable=AsyncMock) as mock_file,
        ):
            mock_module.return_value = MagicMock(spec=[])
            mock_file.return_value = MagicMock(spec=[])
            await factory.load_configured_tools(self._config("/srv/tools"), registry)

        assert mock_module.await_args.kwargs["tools_dir"] == "/srv/tools"
        assert mock_file.await_args.kwargs["tools_dir"] == "/srv/tools"

    @pytest.mark.asyncio
    async def test_unset_tools_dir_falls_back_to_loader_default(self):
        factory = StrandsAgentFactory(container=MagicMock())
        registry = MagicMock()
        ...
        with (
            ...
            mock_module.return_value = MagicMock(spec=[])
            mock_file.return_value = MagicMock(spec=[])
            await factory.load_configured_tools(self._config(None), registry)

        # ``None`` reaches the loader, which resolves it to ``TOOLS_DIR``.
        assert mock_module.await_args.kwargs["tools_dir"] is None
        assert mock_file.await_args.kwargs["tools_dir"] is None

    @pytest.mark.asyncio
    async def test_config_tools_dir_loads_a_real_file_end_to_end(self, tmp_path):
        """A tool file inside a configured ``tools_dir`` loads."""
        from foundry_strands_agent.config.models import AgentModelConfig, StrandsAgentConfig

        tool_file = tmp_path / "greet.py"
        tool_file.write_text(
            'TOOL_SPEC = {"name": "greet", "description": "Greet.", "inputSchema": {"json": {}}}\n\n\n'
            'def greet(name: str) -> str:\n    """Greet someone."""\n    return "hello " + name\n'
        )
        config = StrandsAgentConfig(
            model=AgentModelConfig(),
            tools_files=[str(tool_file)],
        ...
        loaded = registry.register_tool.call_args[0][0]
        assert loaded.TOOL_SPEC["name"] == "greet"
        assert loaded.greet("world") == "hello world"
