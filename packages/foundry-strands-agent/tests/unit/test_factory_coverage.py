...
class TestConfigToDict:
    ...
    def test_includes_bedrock_temperature_rejecting_models(self, factory_with_config):
        factory, _, _ = factory_with_config
        config = StrandsAgentConfig(model=AgentModelConfig(bedrock_temperature_rejecting_models=["xai.grok-4.6"]))
        result = factory._config_to_dict(config)
        assert result["model"]["bedrock_temperature_rejecting_models"] == ["xai.grok-4.6"]
