...
from foundry_agent_core import _MAX_CONTENT_LEN, _MAX_QUERY_LEN
...
class TestQueryRequest:
    ...
    def test_query_too_long_raises(self):
        ...
        with pytest.raises(ValidationError):
            QueryRequest(query="x" * (_MAX_QUERY_LEN + 1))

    def test_query_between_old_and_new_bound_succeeds(self):
        """A query beyond the old 8,192-char bound is now accepted."""
        request = QueryRequest(query="x" * 100_000)
        assert len(request.query) == 100_000
...
class TestDocumentChunk:
    ...
    def test_character_end_at_max_document_size_passes(self):
        """character_end at the canonical document-size bound is accepted."""
        chunk = DocumentChunk(
            chunk_id="chunk-1",
            content="Content",
            metadata={},
            source_document_id="doc-1",
            chunk_index=0,
            character_start=_MAX_DOCUMENT_SIZE - 1,
            character_end=_MAX_DOCUMENT_SIZE,
        )
        assert chunk.character_end == _MAX_DOCUMENT_SIZE

    def test_character_end_above_max_document_size_raises(self):
        """Test that character_end beyond the document-size bound raises ValueError."""
        with pytest.raises(ValidationError):
            DocumentChunk(
                chunk_id="chunk-1",
                content="Content",
                ...
                chunk_index=0,
                character_start=0,
                character_end=_MAX_DOCUMENT_SIZE + 1,
            )
...
class TestQueryResponse:
    ...
    def test_response_text_too_long_raises(self):
        ...
        with pytest.raises(ValidationError):
            QueryResponse(
                response_text="x" * (_MAX_CONTENT_LEN + 1),
                sources=[],
                chunks_used=[],
                confidence_score=0.5,
                processing_time_ms=10.0,
    ...
    def test_response_text_at_max_tokens_ceiling_succeeds(self):
        """response_text sized to the max_tokens=131072 ceiling (4:1 ratio) is accepted."""
        response = QueryResponse(
            response_text="x" * (131_072 * 4),
            sources=[],
            chunks_used=[],
            confidence_score=0.5,
            processing_time_ms=10.0,
        )
        assert len(response.response_text) == 131_072 * 4
